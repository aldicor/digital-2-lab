...
static char buf[64];
static int len;
static u32 estado_gpio = 0; // Ã¢â€ ï¿½ salida actual canal 1
int alerta;
int bloqueado;
// Prototipos
...
// --- FunciÃƒÂ³n auxiliar para modificar una salida sin afectar las otras ---
void setGpioBit(u32 mask, int state) {
    if (state)
        estado_gpio |= mask;
    else
...
int main(void) {
    ...
    // Canal 1 = salida (alerta y huella vÃƒÂ¡lida)
...
// --- VerificaciÃƒÂ³n de huella digital ---
int verificacionHuella(void) {
    static int tries = 0;
...
u32 seconds() {
    XTime t;
    XTime_GetTime(&t);  // lee el nÃƒÂºmero de ciclos
